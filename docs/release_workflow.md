...
本文档说明 `qisheng_player` 的 Windows 发布约定。发布前应先同步更新 `pubspec.yaml` 与 `lib/app_settings.dart` 中的版本号。
...
```powershell
powershell -ExecutionPolicy Bypass -File tools/release/package_release_windows.ps1 -Version <version> -ReuseExistingPackage
```

输出文件：

- `dist/windows/artifacts/packages/Qisheng-Player-v<version>-Windows-x64.zip`
- `dist/windows/artifacts/packages/Qisheng-Player-v<version>-Setup-x64.exe`
...
- Tag：`v<version>`
- Release 标题：`v<version>`
- Release 说明来源：`docs/releases/v<version>.md`
