# Qisheng Player v1.7.1

本文件保留旧版本线的预发布说明，原始发布 tag 为 `v1.7.1`。
...
- Now Playing 竖向、横向和沉浸歌词增强当前行层级、透明度、字号和 glow，快速切歌时清理旧滚动与延迟任务。
- 自定义背景图存在时，流体背景只作为 tint / scrim，不覆盖用户图片。
...
- 已执行 `flutter analyze`、`flutter test`、`flutter test tools\test\sort_smoke_test.dart`、`cargo check`、`flutter build windows --debug`、`flutter build windows --release`。

## 下载内容

- 安装程序：`Coriander-Player-v1.7.1-Setup-x64.exe`
- 便携压缩包：`Coriander-Player-v1.7.1-Windows-x64.zip`
