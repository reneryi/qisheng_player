...
* **技术实现方案**：
  1. **4~6 动态色彩锚点网格（Dynamic Mesh Points）**：
     * 从专辑封面提取的主色、次高亮色、副色、暗环境色作为 4~6 个光斑球中心；
  2. **SkSL / GLSL 片段着色器 (`shaders/mesh_flow.frag`)**：
     * 计算每个像素与各色彩锚点的加权高斯距离（Gaussian Distance Weighting）；
     * 加入低频平滑 Simplex Noise 扰动，让光斑产生如丝绸流体般的缓慢推移与呼吸变幻；
     * 色彩在线性空间（Linear Color Space）中进行 Gamma 矫正混合，根据当前 Brightness 自动插值光斑强度权重。

---

### 2.5 交互水波纹 (Interactive Water Ripple)
* **视觉定义**：将播放器底板化作一汪宁静幽深的水面（完全不透后方窗口），具备极佳的物理交互感与视听共振。
* **独立环境特性（关键细节）**：
  * **无视明暗模式切换**：交互水波纹拥有独立、自洽的水体光学空间，不随系统明暗模式切换而变为刺眼的白水池，恒久保持清幽、空灵的水光质感与深色湖面底色。
* **三大交互响应机制**：
  1. **鼠标滑行微澜（Mouse Trail Waves）**：使用 `Listener` 捕获鼠标在窗口内的移动坐标与速度向量，在着色器中生成细腻的跟随波痕；
  2. **鼠标点击激荡（Click Shockwave）**：点击任意区域时触发高振幅衰减正弦扩散水波（Damped Sine Ripple）；
  3. **低音重击共鸣（Bass-Reactive Resonance）**：在大播放页模式下，接入 BASS 音频引擎的低频能量（FFT Sub-Bass 0~120Hz），每次遇到鼓点（Kick/Bass）重击，从封面中心激荡出一圈向外扩散的同心水波涟漪！
* **技术实现方案**：
  * 编写 `shaders/water_ripple.frag` 着色器，使用波动方程扰动 UV 坐标与环境法线（Normal Perturbation），结合环境光高光（Specular Highlight）渲染逼真水面折射。
...
* **升级方案**：
  1. **多调色板提取（5-Color Harmonic Palette）**：
     * 从专辑封面提取五种语义色：
       ...
       * `highlightGlow`（高光点缀色）
  2. **色彩清洗与色彩空间转换（Chroma & Lightness Clamping）**：
     * 自动过滤低饱和度灰色（Saturation < 0.12 且不是纯黑白）；
     * 在 Oklab / HSL 空间中做对比度提升（Chroma Boosting），使输入给 Mesh Shader 的颜色纯正、通透、富有生命力。
