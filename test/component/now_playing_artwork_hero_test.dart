...
void main() {
  ...
  group('NowPlayingArtworkCard', () {
    ...
    testWidgets('maintains previous cover and avoids flashing placeholder while new track cover is loading', (tester) async {
      final mockProviderA = MemoryImage(Uint8List.fromList([
        0x89, 0x50, 0x4E, 0x47, 0x0D, 0x0A, 0x1A, 0x0A, 0x00, 0x00, 0x00, 0x0D,
        0x49, 0x48, 0x44, 0x52, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01,
        0x08, 0x06, 0x00, 0x00, 0x00, 0x1F, 0x15, 0xC4, 0x89, 0x00, 0x00, 0x00,
        0x0A, 0x49, 0x44, 0x41, 0x54, 0x78, 0x9C, 0x63, 0x00, 0x01, 0x00, 0x00,
      ...
      final audioA = TestAudio(
        title: 'Song A',
        artist: 'Artist A',
        album: 'Album A',
        path: '/path/song_a.mp3',
      );
      NowPlayingArtworkCard.cacheSyncCover(audioA, mockProviderA);

      final audioB = TestAudio(
        title: 'Song B',
        artist: 'Artist B',
        album: 'Album B',
        path: '/path/song_b.mp3',
      );

      // Initially render audioA with known cover
      await tester.pumpWidget(
        _buildTestApp(
          SizedBox(
            width: 100,
      ...
      // Now switch to audioB (whose cover is not yet loaded)
      await tester.pumpWidget(
        _buildTestApp(
          SizedBox(
            width: 100,
      ...
      // Immediately after switching, previous cover is smoothly retained:
      // Image widget remains visible without ever showing the music note placeholder!
      await tester.pump();
      expect(find.byIcon(Symbols.music_note), findsNothing);
      expect(find.byType(Image), findsOneWidget);

      // Once new cover resolves, image remains seamlessly rendered without placeholder
