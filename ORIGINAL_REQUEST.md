...
## 2026-09-16T15:04:12Z

Requested team: Full multi-agent team (完整多智能体团队)

重构与美化栖声播放器的“音乐编辑”功能体系：彻底解决编辑后底层音频文件标签及内嵌封面无法真正持久化、播放器无法即时热更新的问题；重做并美化音乐编辑弹窗交互；修复多在线音乐源检索封面和歌词解析成功率低的问题；建立清晰合理的单曲、专辑与艺术家视觉封面边界。
...
### R1. 重构与美化“音乐编辑”对话框 UI/UX
- 废弃当前简陋、生硬的表单与零散复选框设计，采用经典双栏现代弹窗布局（对齐项目现有 ModernDialogFrame 与材质风格）：
  - 左栏：歌曲封面管理（支持清晰大图预览、本地选取图片替换、移除封面）与核心元数据表单（歌名、艺术家、专辑、专辑艺术家、音轨号、碟号等）。
  - 右栏：智能在线搜索与候选匹配列表，支持一键采纳匹配结果（自动填充标签、关联高画质封面与歌词）并提供实时歌词预览与微调。
- 交互直观流畅，具有清晰的保存、放弃与加载状态反馈，杜绝令人困惑的属性单独勾选逻辑。

### R2. 底层音频元数据真实物理持久化与全局即时同步
- 支持将用户修改的元数据（标题、艺术家、专辑、专辑艺术家、音轨、碟号、内嵌歌词与内嵌封面图片）真正、可靠地写入底层音频文件本身（主流格式如 MP3、FLAC、M4A/AAC、OGG 等），CUE 虚拟分轨提供合理覆盖存储。
- 解决文件占用和写入冲突，保存成功后立即触发曲库（AudioLibrary）、播放列表（Playlists）、底栏正在播放（PlaybackService）及详情页的即时热更新，彻底解决“改完后播放器依旧显示旧标签错误信息”的问题。

### R3. 升级与修复在线元数据及封面检索源
- 诊断并修复网易云音乐、QQ音乐、酷狗等音源在搜索阶段封面 URL 容易缺失、低清或失效的顽疾，通过必要的数据补全机制（如歌曲/专辑详情拉取）确保搜索候选列表具备高可用封面与准确歌词。
- 优先确保基础音乐的“歌名/歌手/专辑”、“内嵌封面”与“歌词”在线检索和应用的极高稳定性与可用性。

### R4. 规范艺术家与专辑视觉呈现体系
- 规范单曲内嵌封面与独立艺术家头像、专辑封面的视觉数据边界，杜绝粗暴地将单曲封面机械套用为艺术家头像或专辑封面。
- 在保证单曲编辑核心主链路稳定可用的前提下，完善艺术家和专辑维度的封面检索与独立管理能力。

## Acceptance Criteria

### 视觉交互规范
- [ ] 音乐编辑弹窗采用统一的经典双栏现代设计，消除文本框前置复选框等怪异控件，视觉与交互风格与主播放器无缝统一。
- [ ] 在线匹配结果清晰展示来源平台标签、标题、艺术家、专辑及封面缩略图，并提供一键智能采纳按钮。

### 文件物理写入与状态更新
- [ ] 编辑普通音频文件（如 FLAC/MP3）的元数据（歌名、歌手、专辑、内嵌封面）并点击保存后，音频文件的物理标签被切实写入。
- [ ] 保存成功后，播放器界面无需重启即可即时看到新标签和新封面生效（包括正在播放底栏、播放列表与曲库列表）。

### 在线检索与匹配质量
- [ ] 在线搜索常见歌曲时，候选列表中网易云、QQ音乐等主要源均能稳定展示对应封面预览图片，且点击采纳能正确载入歌词与封面。
- [ ] 采纳在线候选信息后点击保存，标签与封面能完整持久化并更新。

### 自动化验证与质量保障
- [ ] 运行 `flutter test` 时相关组件和库测试全部通过无报错。
- [ ] 在无网络或网络异常时，编辑弹窗具备优雅的降级与错误提示，不产生未捕获异常或界面假死。
