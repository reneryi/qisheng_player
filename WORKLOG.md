...
## 2026-07-25 - qisheng_player v1.3.1 窗口尺寸恢复修复

### 背景

- Windows 无边框窗口在最小尺寸 `507 × 507` 下缺少可靠的 Flutter 边缘缩放兜底，用户只能通过最大化或全屏改变尺寸。
- 配置虽然记录了最大化状态，但启动流程没有恢复该状态；保存的普通窗口尺寸为最小值时，每次重启都会再次打开为最小窗口。

### 实现

- Windows 启用 `WindowResizeFrame`，通过现有原生通道支持四边和四角拖动缩放。
- 启动时恢复上次最大化状态，普通窗口缩放结束后防抖保存尺寸，并在最大化或全屏期间保留最后一次普通尺寸。
- 对 `WindowSize` 增加解析与边界校验，异常值回退为默认窗口尺寸。
- 默认窗口尺寸调整为 `1461 × 898`，版本升级为 `1.3.1`。

### 验证

- Flutter 静态分析通过，全量 `142` 项测试通过。
- Windows Release 构建、便携压缩包和 Inno Setup 安装器生成通过。
